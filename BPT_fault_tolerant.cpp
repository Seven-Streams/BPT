...
template <class W, int info_len = 3> class MemoryRiverPlus;
...
public:
  ...
  void ChangeName2(std::string res) {
    file_name = res;
    initialise_donothing();
  }
  void
  ...
  friend class MemoryRiverPlus<W, info_len>;
  friend class MemoryRiverPlus<W, info_len + 1>;
};
template <class W, int info_len> class MemoryRiverPlus {
private:
  MemoryRiver<W, info_len> datas;
...
template <class Value = int, int size = 200> class BPT {
private:
  struct MyData {
  ...
  MemoryRiverPlus<Node, 3> mydatabase;
...
public:
  BPT() = default;
  BPT(std::string name) : mydatabase(name){};
  ~BPT() = default;
